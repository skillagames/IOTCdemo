...
import { deviceService, Device, UsageStat } from "../services/deviceService";
import {
  Activity,
  Clock,
  Database,
  ...
  const PLANS = [
    {
      ...
      name: "Starter Pulse",
      price: "R 79.00",
      desc: "Basic connectivity for 30 days",
    },
    {
      id: 1,
      name: "Monthly Booster",
      price: "R 159.00",
      desc: "Unlimited data for 30 days",
      badge: "Popular",
    },
    {
      id: 2,
      name: "Quarterly Core",
      price: "R 399.00",
      desc: "Solid performance for 90 days",
    },
    {
      id: 3,
      name: "Annual Pro",
    ...
    <div className="space-y-3">
        ...
        <div className="mt-4 flex items-center justify-between border-t border-slate-50 pt-3">
          <div className="flex flex-col">
            ...
            <p className="text-sm font-black text-slate-900 mt-1.5 leading-none">
              {isInactive
                ? "Pending Activation"
                : formatDate(device.expirationDate, "MMMM dd, yyyy")}
            </p>
          </div>
      ...
      <section className="space-y-2.5">
        <div className="flex items-center justify-between px-1">
          <h3 className="text-[8px] font-black uppercase tracking-[0.2em] text-slate-400 leading-none">
            Usage Analytics
          </h3>
          <button
        ...
        <div className="grid grid-cols-2 gap-3">
          <DetailBadge
            icon={Database}
            label="Total Data"
            value={`${stats.reduce((acc, s) => acc + s.dataUsedMb, 0).toLocaleString()} MB`}
          />
          <DetailBadge
            icon={Clock}
            label="Active Time"
            value={`${stats.reduce((acc, s) => acc + s.activeHours, 0)} Hrs`}
          />
        </div>
      </section>
...
const PlanOption = ({
  ...
  >
    ...
    <div className="min-w-0">
      <h4
        className={cn(
          "font-black text-[10px] uppercase tracking-wider truncate",
